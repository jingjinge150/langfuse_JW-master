// src/components/charts/BigNumber.jsx
...
// CSS classes utility function (cn 대체)
...
export const BigNumber = ({ data, className = "" }) => {
  ...
  const calculatedMetric = useMemo(() => {
    ...
    if (data.length === 1) {
      return typeof data[0].metric === "number"
        ? data[0].metric
    ...
    >
      <div className="flex items-baseline justify-center gap-1">
        ...
        {displayValue.unit && (
          <span
            className={combineClasses(
              "font-bold text-gray-500", // text-muted-foreground 대체
              fontSize === "text-8xl"
                ? "text-4xl"
                : fontSize === "text-7xl"
                ? "text-3xl"
